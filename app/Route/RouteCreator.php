...
class RouteCreator
{
    ...
    {
        if (isset($this->paths[$from->getPlaceId() . ':' . $to->getPlaceId()])) {
            return $this->paths[$from->getPlaceId() . ':' . $to->getPlaceId()];
        }

        $response = \GoogleMaps::load('directions')
            ->setParam(['origin' => 'place_id:' . $from->getPlaceId()])
            ->setParam(['destination' => 'place_id:' . $to->getPlaceId()])
            ->setParam(['mode' => 'walking'])
            ->get();
        $distance = json_decode($response)->routes[0]->legs[0]->distance->value;
        $this->paths[$from->getPlaceId() . ':' . $to->getPlaceId()] = $distance;
        return $distance;
    }
}